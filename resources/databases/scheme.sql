...
CREATE TABLE t_user_ai_provider_config
(
    id              VARCHAR(20)  NOT NULL PRIMARY KEY,
...
CREATE INDEX idx_user_ai_provider_user ON t_user_ai_provider_config (user_id, enabled);
COMMENT ON TABLE t_user_ai_provider_config IS '用户AI提供商配置';
COMMENT ON COLUMN t_user_ai_provider_config.user_id IS '用户ID';
COMMENT ON COLUMN t_user_ai_provider_config.provider IS '提供商标识';
COMMENT ON COLUMN t_user_ai_provider_config.base_url IS 'OpenAI兼容接口基础地址';
COMMENT ON COLUMN t_user_ai_provider_config.api_key IS '提供商API Key';
COMMENT ON COLUMN t_user_ai_provider_config.chat_model IS '聊天模型';
COMMENT ON COLUMN t_user_ai_provider_config.embedding_model IS '向量模型';
COMMENT ON COLUMN t_user_ai_provider_config.verified IS '是否验证通过 0：否 1：是';
COMMENT ON COLUMN t_user_ai_provider_config.last_verified_at IS '最近验证时间';
COMMENT ON COLUMN t_user_ai_provider_config.enabled IS '是否启用 0：否 1：是';
